...
//This is a basic vmod to count the number of needles in haystack
int vmod_count(char haystack[], char needle[]){
	char* need = needle; //copy of needle
	int ret = 0; //return variable
	while(haystack != NULL){ //reads through haystack until the last character.
		if(need == NULL){ //needle parsed through and found in the haystack
			ret = ret+1;
			need = needle; //resetting need
		}
		if(need == haystack){ //finding that the first character of needle is found in haystack
			need++;
		}
		haystack++;
	}
